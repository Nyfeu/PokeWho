...
class MotorInferenciaEventDriven:
    def __init__(self, caminho_yaml):
        ...
        # Embaralha a agenda para evitar viés posicional
    ...
    def registrar_resposta(self, fato, resposta_afirmativa):
        if resposta_afirmativa:
            self.fatos.add(fato)
            # EXCLUSÃO MÚTUA: Atributos da mesma categoria são invalidados
    ...
    def _inferir(self):
        ...
        # DEDUÇÃO POR ELIMINAÇÃO: Realizada internamente após o encadeamento
        regras_ativas = self._regras_viaveis()
